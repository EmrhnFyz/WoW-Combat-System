...
{
    [Flags]
    private enum PackageFlags
    {
        WarnForProjectOverwrite = 1 << 1,
        RunInitialSetup = 1 << 2
    }

    private class BoltWizardText
    {
...
Please, feel free to click on the Next button, and get started.";
    ...
    private Boolean MainPackageInstalled()
    {
        var SETTINGS_PATH = Path.Combine(BoltPathUtility.ResourcesPath, "BoltRuntimeSettings.asset");
        var PREFABDB_PATH = Path.Combine(BoltPathUtility.ResourcesPath, "BoltPrefabDatabase.asset");

        return File.Exists(SETTINGS_PATH) && File.Exists(PREFABDB_PATH);
    }

    private Boolean SamplesPackageInstalled()
    {
        return Directory.Exists("Assets/samples");
    }

    private Boolean XB1PackageInstalled()
    {
        return File.Exists("Assets/Plugins/XB1.dll");
    }

    private Boolean PS4PackageInstalled()
    {
        return File.Exists("Assets/Plugins/PS4.dll");
    }

    private Boolean MobilePackageInstalled()
    {
        return Directory.Exists("Assets/Plugins/iOS") && Directory.Exists("Assets/Plugins/Android");
    }

    private Action OpenURL(String url, params System.Object[] args)
    {
        return () =>
        {
            if (args.Length > 0)
    ...
    private static bool IsAppId(string val)
    {
        try
        {
...
#pragma warning restore RECS0026 // Possible unassigned object created by 'new'
    ...
    private void PrepareReleaseHistoryText()
    {
        ...
        MatchCollection matches = regexVersion.Matches(baseText);

        if (matches.Count > 0)
        {
            Match currentVersionMatch = matches[0];
            Match lastVersionMatch = currentVersionMatch.NextMatch();
            ...
            if (currentVersionMatch.Success && lastVersionMatch.Success)
            {
                ...
                MatchCollection resultAdded = regexAdded.Matches(mainText);
                MatchCollection resultChanged = regexChanged.Matches(mainText);
                MatchCollection resultFixed = regexFixed.Matches(mainText);
                MatchCollection resultRemoved = regexRemoved.Matches(mainText);
