    ...
    internal class WarcraftCharacterController : EntityBehaviour<IUnitState>, IUnitBehaviour
    {
        ...
        void IUnitBehaviour.DoUpdate(int deltaTime)
        {
            if (IsMovementController)
            {
                ...
                inputProvider.PollInput(unit, out inputVelocity, out Quaternion inputRotation, out var shouldJump);
                ...
                var movingRight = rawInputVelocity.x > 0;
                var movingLeft = rawInputVelocity.x < 0;
        ...
        {
            var isMovingLocally = IsMovementController;
            var isKinematic = !isMovingLocally || unit.AI.NavMeshAgentEnabled || unit.Motion.UsesKinematicMovement;
        ...
        private void ProcessMovement()
        {
            ...
            void ProcessGroundState()
            {
                ...
                else
                {
                    ...
                    var farFromGround = !hasGroundHit || Mathf.Abs(unit.UnitCollider.bounds.center.y - lastGroundHitInfo.point.y - unit.UnitCollider.bounds.extents.y) > IdleGroundDistance;
                    var inNonGroundedState = !TouchingGround || unit.HasMovementFlag(MovementFlags.MaskAir);
