...
namespace Core
{
    ...
    public partial class Spell
    {
        internal void EffectTeleportDirect(EffectTeleportDirect effect, int effectIndex, Unit target, SpellEffectHandleMode mode)
        {
            ...
            var topCheck = Math.Abs(target.UnitCollider.bounds.max.y);
            var safeExtentsY = Math.Abs(target.UnitCollider.bounds.extents.y);
            var safeExtentsX = Math.Abs(target.UnitCollider.bounds.extents.x);
            var distance = effect.HorizontalDistance;
            ...
            {
                targetPosition += target.transform.forward * distance;
            }
