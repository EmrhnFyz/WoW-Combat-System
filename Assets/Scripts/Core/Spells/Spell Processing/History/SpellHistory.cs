...
namespace Core
{
    ...
    {
        private readonly Dictionary<int, SpellCooldown> spellCooldownsById = new();
        private readonly Dictionary<int, List<SpellChargeCooldown>> spellChargesById = new();
        private readonly List<SpellCooldown> spellCooldowns = new();
        ...
        internal void DoUpdate(int deltaTime)
        {
            ...
            foreach (KeyValuePair<int, List<SpellChargeCooldown>> chargeEntry in spellChargesById)
            {
                ...
                while (chargeEntry.Value.Count > 0 && remainingDeltaTime > 0)
                {
                    ...
                    var handledDelta = Mathf.Min(currentChargeCooldown.ChargeTimeLeft, remainingDeltaTime);
        ...
        {
            var expectedCooldownFrames = (int)(cooldownEvent.CooldownTime / BoltNetwork.FrameDeltaTime / 1000.0f);
            var framesPassed = BoltNetwork.ServerFrame - cooldownEvent.ServerFrame;
            ...
            var cooldownTimeLeft = Mathf.RoundToInt(cooldownEvent.CooldownTime * cooldownProgressLeft);
        ...
        public void Handle(SpellChargeEvent chargeEvent)
        {
            ...
            var expectedCooldownFrames = (int)(chargeEvent.CooldownTime / BoltNetwork.FrameDeltaTime / 1000.0f);
            var framesPassed = BoltNetwork.ServerFrame - chargeEvent.ServerFrame;
            ...
            var cooldownProgressLeft = 1.0f - ((float)framesPassed / expectedCooldownFrames);
            var cooldownTimeLeft = Mathf.RoundToInt(chargeEvent.CooldownTime * cooldownProgressLeft);
        ...
        internal void HandleCooldown(SpellInfo spellInfo)
        {
            ...
            var cooldownLeft = spellInfo.CooldownTime;
            if (cooldownLeft <= 0)
            {
                return;
            }
        ...
        private void OnGlobalCooldownChanged()
        {
            ...
            var expectedGlobalFrames = (int)(GlobalCooldown / BoltNetwork.FrameDeltaTime / 1000.0f);
            if (expectedGlobalFrames > 0)
            {
                var globalServerFrame = casterState.GlobalCooldown.ServerFrame;
                var cooldownLeftRatio = 1 - Mathf.Clamp01((float)(BoltNetwork.ServerFrame - globalServerFrame) / expectedGlobalFrames);
