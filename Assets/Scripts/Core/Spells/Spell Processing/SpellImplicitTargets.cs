...
namespace Core
{
    internal class SpellImplicitTargets
    {
        ...
        private readonly Dictionary<Unit, SpellTargetEntry> targetEntryByTarget = new();
        ...
        internal void HandleLaunch(out bool isDelayed, out SpellProcessingToken processingToken)
        {
            ...
            foreach (SpellTargetEntry targetEntry in Entries)
            {
                ...
                {
                    var distance = Mathf.Clamp(Vector3.Distance(spell.Caster.Position, targetEntry.Target.Position), StatUtils.DefaultCombatReach, float.MaxValue);
