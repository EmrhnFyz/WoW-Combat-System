...
namespace Core
{
    public partial class Spell
    {
        ...
        {
            if (ExecutionState is SpellExecutionState.Preparing or SpellExecutionState.Casting)
            {
                Finish();
            }
        }
        ...
        internal void DoUpdate(int deltaTime)
        {
            switch (ExecutionState)
            {
                case SpellExecutionState.Casting:
                    ...
                    var mayBeInterruptedByMove = CastTime - CastTimeLeft > MovementUtils.SpellMovementInterruptThreshold;
        ...
        internal void AddAppliedModifier(SpellModifier spellModifier)
        {
            ...
            for (var i = 0; i < spellModifier.Aura.Effects.Count; i++)
            {
        ...
        internal void HandleUnappliedModifers(Unit unit, SpellModifierType modifierType, SpellModifierApplicationType applicationType, ref float value)
        {
            switch (applicationType)
            {
                case SpellModifierApplicationType.Flat:
                    for (var i = unappliedModifiers.Count - 1; i >= 0; i--)
                    {
        ...
        private SpellCastResult ValidateAuras()
        {
            ...
            var usableWhileStunned = SpellInfo.HasAttribute(SpellExtraAttributes.UsableWhileStunned);
            var usableWhileConfused = SpellInfo.HasAttribute(SpellExtraAttributes.UsableWhileConfused);
            var usableWhileFeared = SpellInfo.HasAttribute(SpellExtraAttributes.UsableWhileFeared);
        ...
        private SpellCastResult ValidateRange()
        {
            ...
            return SpellInfo.ExplicitTargetType switch
            {
                SpellExplicitTargetType.None => SpellCastResult.Success,
                SpellExplicitTargetType.Caster => SpellCastResult.Success,
                SpellExplicitTargetType.Target => ValidateTargetRange(),
                SpellExplicitTargetType.Destination => ValidateDestinationRange(),
            ...
            SpellCastResult ValidateDestinationRange()
            {
                ...
                var minRange = SpellInfo.GetMinRange(false);
        ...
        private SpellCastResult ValidateShapeShift()
        {
            ...
            if (shapeShiftIgnoreEffects != null)
            {
                for (var i = 0; i < shapeShiftIgnoreEffects.Count; i++)
                {
        ...
        private SpellCastResult ValidateMechanics(AuraEffectType auraEffectType)
        {
            ...
            for (var i = 0; i < activeEffects.Count; i++)
            {
                ...
                {
                    return auraEffectType switch
                    {
                        AuraEffectType.StunState => SpellCastResult.Stunned,
                        AuraEffectType.ModFear => SpellCastResult.Fleeing,
                        AuraEffectType.ConfusionState => SpellCastResult.Confused,
        ...
        private SpellCastResult Cast()
        {
            ...
            var instantCast = CastTime <= 0.0f || Caster.SpellCast.IsCasting || spellValue.CastFlags.HasTargetFlag(SpellCastFlags.CastDirectly);
        ...
        private void Launch()
        {
            ...
            ImplicitTargets.HandleLaunch(out var isDelayed, out SpellProcessingToken processingToken);
        ...
        private void PrepareExplicitTarget()
        {
            ...
            var targetsUnits = SpellInfo.ExplicitCastTargets.HasAnyFlag(SpellCastTargetFlags.UnitMask);
        ...
        private void SelectImplicitTargets()
        {
            ...
            var processedAreaEffectsMask = 0;
            for (var effectIndex = 0; effectIndex < SpellInfo.Effects.Count; effectIndex++)
            {
                ...
                var effectMask = 1 << effectIndex;
                for (var otherEffectIndex = 0; otherEffectIndex < SpellInfo.Effects.Count; otherEffectIndex++)
                {
            ...
            void SelectRedirectedTargets()
            {
                ...
                {
                    Unit redirectTarget = SpellInfo.DamageClass switch
                    {
                        SpellDamageClass.Magic => Caster.Spells.GetMagicHitRedirectTarget(target, SpellInfo),
                        SpellDamageClass.Melee or SpellDamageClass.Ranged => Caster.Spells.GetMeleeHitRedirectTarget(target, SpellInfo),
                        _ => null,
                    };
