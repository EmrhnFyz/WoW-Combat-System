...
namespace Core
{
    public class Aura
    {
        ...
        private readonly List<Unit> tempRemovableTargets = new();
        private readonly HashSet<Unit> tempUpdatedTargets = new();
        private readonly List<AuraEffect> effects = new();
        private readonly List<AuraEffectInfo> effectInfos = new();
        private readonly List<AuraApplication> applications = new();
        private readonly Dictionary<ulong, AuraApplication> applicationsByTargetId = new();
        ...
        [CanBeNull] public Unit Caster => casterReference.Value;
        [NotNull] public Unit Owner { get; }
        [NotNull] public AuraInfo AuraInfo { get; }
        [NotNull] public SpellInfo SpellInfo { get; }
        ...
        internal Aura(AuraInfo auraInfo, SpellInfo spellInfo, [NotNull] Unit owner, [NotNull] Unit caster)
        {
            ...
            for (var index = 0; index < effectInfos.Count; index++)
            {
        ...
        internal void UpdateTargets()
        {
            ...
            for (var i = applications.Count - 1; i >= 0; i--)
            {
        ...
        internal bool CanStackWith(Aura existingAura)
        {
            ...
            var sameCaster = CasterId == existingAura.CasterId;
        ...
        private void AddUnitToAura(Unit unit)
        {
            ...
            if (unit != Owner)
            {
                for (var i = 0; i < unit.AuraApplications.Count; i++)
                {
