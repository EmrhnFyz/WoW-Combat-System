...
namespace Core.AuraEffects
{
    public class AuraEffectPeriodicDamage : AuraEffectPeriodic
    {
        ...
        protected override void HandlePeriodic(Unit target, Unit caster)
        {
            ...
            {
                var damageInfo = new SpellDamageInfo(caster, target, Aura.SpellInfo, SpellDamageType.Dot, HitType.Immune);
                EventHandler.ExecuteEvent(GameEvents.ServerDamageDone, damageInfo);
            }
            else
            {
                var originalDamage = EffectInfo.CalculateSpellDamage(caster);
                var hasCrit = caster.Spells.IsSpellCrit(target, Aura.SpellInfo, EffectInfo.SpellSchoolMask);
