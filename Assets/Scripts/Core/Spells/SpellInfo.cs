...
namespace Core
{
    ...
    public sealed class SpellInfo : ScriptableUniqueInfo<SpellInfo>
    {
        ...
        private HashSet<ShapeShiftForm> shapeShiftAlwaysCastableSet = new();
        private HashSet<ShapeShiftForm> shapeShiftNeverCastableSet = new();
        ...
        protected override void OnRegister()
        {
            ...
            foreach (SpellEffectInfo spellEffectInfo in spellEffectInfos)
            {
                ...
                {
                    for (var index = 0; index < auraApplyEffect.AuraInfo.AuraEffects.Count; index++)
                    {
        ...
        public bool CanPierceImmuneAura(SpellInfo spellInfo)
        {
            ...
            return spellInfo.Mechanic is not (SpellMechanics.ImmuneShield or SpellMechanics.Invulnerability or SpellMechanics.Banish);
        }
        ...
        public bool CanCancelAura(AuraEffect auraEffect)
        {
            ...
            {
                if (effectInfo is not EffectApplyAura applyAuraEffect)
                {
                    continue;
                }

                for (var index = 0; index < applyAuraEffect.AuraInfo.AuraEffects.Count; index++)
                {
        ...
        public bool CanCancelForm(Spell spell)
        {
            ...
            var impossibleToCancel = spell.Caster.ShapeShiftSpellInfo.HasAttribute(SpellAttributes.CantCancel);
            var mayNotCancel = HasAttribute(SpellExtraAttributes.CastCantCancelShapeShift);
        ...
        public SpellCastResult CheckTarget(Unit caster, Unit target, Spell spell, bool isImplicit = true)
        {
            ...
            if (HasAttribute(SpellAttributes.OnlyTargetPlayers) && target is not Player)
            {
        ...
        public SpellCastResult CheckShapeShift(Unit caster)
        {
            ...
            if (caster.ShapeShiftForm != ShapeShiftForm.None)
            {
                ...
                var impossibleToCancel = caster.ShapeShiftSpellInfo.HasAttribute(SpellAttributes.CantCancel);
                var mayNotCancel = HasAttribute(SpellExtraAttributes.CastCantCancelShapeShift);
        ...
        {
            var range = positive ? MaxRangeFriend : MaxRangeHostile;
        ...
        public void CalculatePowerCosts(Unit caster, List<(SpellPowerType, int)> powerCosts, Spell spell = null)
        {
            ...
            {
                var powerCost = powerCostInfo.PowerCost;
