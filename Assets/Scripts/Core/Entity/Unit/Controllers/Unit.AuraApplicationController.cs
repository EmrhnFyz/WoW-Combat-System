...
namespace Core
{
    public abstract partial class Unit
    {
        internal class AuraApplicationController : IUnitBehaviour
        {
            ...
            private readonly Dictionary<AuraStateType, List<AuraApplication>> auraApplicationsByAuraState = new();
            private readonly Dictionary<AuraEffectType, List<AuraEffect>> auraEffectsByAuraType = new();
            private readonly Dictionary<int, List<Aura>> ownedAurasById = new();
            private readonly Dictionary<int, List<AuraApplication>> auraApplicationsByAuraId = new();
            private readonly Dictionary<int, List<AuraApplication>> auraApplicationsBySpellInfoId = new();
            private readonly List<AuraApplication> interruptableAuraApplications = new();
            private readonly List<AuraApplication> auraApplications = new();
            private readonly HashSet<AuraApplication> auraApplicationSet = new();
            private readonly List<Aura> ownedAuras = new();

            private readonly Dictionary<AuraApplication, AuraRemoveMode> tempApplicationsToRemove = new();
            private readonly List<AuraApplication> tempAuraApplications = new(10);
            ...
            internal bool HasAuraState(AuraStateType auraStateType, Unit caster = null, Spell spell = null)
            {
                ...
                if (caster != null)
                {
                    ...
                    {
                        for (var i = 0; i < ignoreAuraEffects.Count; i++)
                        {
            ...
            internal void ApplyAuraApplication(AuraApplication auraApplication)
            {
                ...
                for (var i = 0; i < auraApplication.Aura.AuraInfo.AuraScriptables.Count; i++)
                {
            ...
            internal void RemoveAurasWithInterrupt(AuraInterruptFlags flags)
            {
                ...
                for (var i = 0; i < interruptableAuraApplications.Count; i++)
                {
            ...
            internal void RemoveAurasWithEffect(AuraEffectType auraEffectType, AuraEffect exceptEffect = null, Aura exceptAura = null, Unit onlyWithCaster = null)
            {
                ...
                List<AuraEffect> auraEffectsToRemove = ListPoolContainer<AuraEffect>.Take();
                for (var i = 0; i < auraEffects.Count; i++)
                {
            ...
            internal void RemoveAurasWithCombinedDamageInterrupt(int damageTaken)
            {
                ...
                for (var i = 0; i < interruptableAuraApplications.Count; i++)
                {
                    ...
                    {
                        interruptableAuraApplications[i].HandleDamageInterrupt(damageTaken, out var removeInstantly);
                        if (removeInstantly)
                        {
                            tempAuraApplications.Add(interruptableAuraApplications[i]);
                        }
            ...
            private void HandleAuraEffects(AuraApplication auraApplication, bool added)
            {
                ...
                if (added)
                {
                    for (var i = 0; i < auraApplication.Aura.Effects.Count; i++)
                    {
                ...
                {
                    for (var i = 0; i < auraApplication.Aura.EffectsInfos.Count; i++)
                    {
                ...
                for (var i = 0; i < auraEffectsToHandle.Count; i++)
                {
            ...
            {
                for (var i = 0; i < auraApplications.Count; i++)
                {
            ...
            {
                for (var i = 0; i < ownedAuras.Count; i++)
                {
                ...
                foreach (KeyValuePair<AuraApplication, AuraRemoveMode> applicationEntry in tempApplicationsToRemove)
                {
