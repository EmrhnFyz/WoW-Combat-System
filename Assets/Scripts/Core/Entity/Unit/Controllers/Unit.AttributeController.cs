        ...
        internal class AttributeController : IUnitBehaviour
        {
            ...
            void IUnitBehaviour.DoUpdate(int deltaTime)
            {
                ...
                if (unit.IsAlive)
                {
                    ...
                    var timeToUpdateDisplay = accumulatedRegenerationTime >= RegenerationSyncTime;

                    foreach (KeyValuePair<SpellPowerType, (int, SpellPowerTypeInfo)> powerEntry in spellPowerIndexes)
                    {
                        var oldValue = powers[powerEntry.Value.Item1, 0].Value;
                        var minValue = powers[powerEntry.Value.Item1, 0].Min;
                        var maxValue = powers[powerEntry.Value.Item1, 1].Value;

                        var regeneratedValue = powerEntry.Value.Item2.Regeneration * deltaTime / 1000;
                        ...
                        var deltaValue = Mathf.FloorToInt(accumulatedRegeneration[powerEntry.Value.Item1]);
                        accumulatedRegeneration[powerEntry.Value.Item1] -= deltaValue;

                        var newValue = Mathf.Clamp(oldValue + deltaValue, minValue, maxValue);
            ...
            void IUnitBehaviour.HandleUnitAttach(Unit unit)
            {
                ...
                void InitializeAttributes()
                {
                    ...
                    else
                    {
                        ...
                        for (var i = 0; i < powers.GetLength(0); i++)
                        {
            ...
            internal int Power(SpellPowerType powerType) => spellPowerIndexes.TryGetValue(powerType, out (int, SpellPowerTypeInfo) i) ? powers[i.Item1, 0].Value : 0;

            internal int MaxPower(SpellPowerType powerType) => spellPowerIndexes.TryGetValue(powerType, out (int, SpellPowerTypeInfo) i) ? powers[i.Item1, 1].Value : 0;

            internal int MaxPowerWithNoMods(SpellPowerType powerType) => spellPowerIndexes.TryGetValue(powerType, out (int, SpellPowerTypeInfo) i) ? powers[i.Item1, 2].Value : 0;
            ...
            {
                var increaseModifier = 0.0f;
                var nonStackModifier = 100f;
                var stackMultiplier = 100f;
                ...
                var speedRate = 1.0f.ApplyPercentage(Mathf.Max(nonStackModifier, stackMultiplier));
                ...
                if (!unit.HasAuraType(AuraEffectType.SpeedSupressSlowEffects))
                {
                    ...
                    var slowPercent = Mathf.Clamp(unit.MaxPositiveAuraModifier(AuraEffectType.SpeedDecreaseModifier), 0.0f, 99.9f);
                    if (slowPercent > 0.0f)
                    {
                ...
                var minSpeedPercent = unit.MaxPositiveAuraModifier(AuraEffectType.ModMinimumSpeed);
            ...
            {
                var totalIntellect = CalculateTotalStatValue(UnitModifierType.Intellect);
            ...
            internal void UpdateMaxPower(SpellPowerType powerType)
            {
                ...
                var value = statModifiers[(unitModifierType, StatModifierType.BaseValue)] + powers[powerTypeInfo.Item1, 1].Base;
            ...
            internal void UpdateAvailablePowers()
            {
                ...
                var maxPowers = powers.GetLength(0);
                var usedPowers = unitClassInfo.PowerTypes.Count;

                Assert.IsTrue(usedPowers <= maxPowers);

                spellPowerIndexes.Clear();
                for (var i = 0; i < usedPowers; i++)
                {
                ...
                for (var i = usedPowers; i < maxPowers; i++)
                {
            ...
            {
                var value = statModifiers[(modType, StatModifierType.BaseValue)];
            ...
            {
                if (spellPowerIndexes.TryGetValue(powerType, out (int, SpellPowerTypeInfo) i))
                {
                    newValue = Mathf.Clamp(newValue, powers[i.Item1, 0].Min, powers[i.Item1, 1].Value);
                    var delta = powers[i.Item1, 0].Set(newValue);
