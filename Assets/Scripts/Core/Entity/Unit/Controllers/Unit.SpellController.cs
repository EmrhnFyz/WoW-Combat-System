...
namespace Core
{
    public abstract partial class Unit
    {
        ...
        {
            private readonly SpellModifierContainer spellModifiers = new();
            private readonly SchoolImmunityContainer schoolImmunities = new();
            private readonly MechanicsImmunityContainer mechanicsImmunities = new();
            private readonly List<AuraEffectSpellTrigger> spellTriggers = new();
            ...
            {
                var spell = new Spell(unit, spellInfo, castOptions);
            ...
            internal void DamageBySpell(SpellDamageInfo damageInfo, Spell spell = null)
            {
                ...
                for (var i = unit.Auras.AuraApplications.Count - 1; i >= 0; i--)
                {
                    AuraApplication application = unit.Auras.AuraApplications[i];

                    for (var j = 0; j < application.Aura.AuraInfo.AuraScriptables.Count; j++)
                    {
            ...
            internal void CalculateSpellHealingTaken(ref SpellHealInfo healInfo)
            {
                ...
                if (healInfo.HasCrit)
                {
                    var criticalHeal = CalculateSpellCriticalHealing(healInfo.Heal);
            ...
            {
                var damageMultiplier = unit.Auras.TotalAuraMultiplier(AuraEffectType.ModifyDamagePercentDone);
            ...
            internal uint SpellDamageBonusTaken(Unit caster, SpellInfo spellInfo, uint damage, SpellDamageType damageType)
            {
                ...
                var damageMultiplier = unit.Auras.TotalAuraMultiplier(AuraEffectType.ModifyDamagePercentTaken);
                var resultingDamage = damage * damageMultiplier;
            ...
            {
                var critChance = CalculateSpellCriticalChance(victim, spellInfo, schoolMask, spell);
            ...
            private float CalculateSpellCriticalChance(Unit victim, SpellInfo spellInfo, SpellSchoolMask schoolMask, Spell spell = null)
            {
                ...
                var critChance = unit.CritPercentage;
                if (victim == null)
                {
                ...
                {
                    for (var i = 0; i < spellCritAuras.Count; i++)
                    {
            ...
            internal void HandleAbsorb(ref SpellDamageInfo damageInfo)
            {
                ...
                for (var index = 0; index < absorbEffectCopies.Count; index++)
                {
                    ...
                    var availableAbsorb = (uint)Mathf.CeilToInt(absorbEffect.Value);
                    var effectiveAbsorb = System.Math.Min(availableAbsorb, damageInfo.Damage);
            ...
            internal void HandleAbsorb(ref SpellHealInfo healInfo)
            {
                ...
                for (var index = 0; index < absorbEffectCopies.Count; index++)
                {
                    ...
                    var availableAbsorb = (uint)Mathf.CeilToInt(absorbEffect.Value);
                    var effectiveAbsorb = System.Math.Min(availableAbsorb, healInfo.Heal);
            ...
            internal bool IsImmuneToSpellSchool(SpellSchoolMask schoolMask, SpellInfo spellInfo, Unit caster)
            {
                if (schoolMask != 0)
                {
                    ...
                    foreach (KeyValuePair<SpellSchoolMask, List<SpellInfo>> schoolImmunityEntry in schoolImmunities)
                    {
            ...
            internal bool IsAffectedBySpellModifier(Spell spell, SpellModifier modifier)
            {
                ...
                for (var i = 0; i < modifier.AuraModifier.ApplicationConditions.Count; i++)
                {
            ...
            internal float ApplySpellModifier(Spell spell, SpellModifierType modifierType, float baseValue)
            {
                ...
                var flatValue = baseValue + valueModifier;
