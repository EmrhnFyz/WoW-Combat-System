...
namespace Core
{
    ...
    {
        [SerializeField, Header(nameof(Unit)), Space(10)] private CapsuleCollider unitCollider;
        [SerializeField] private WarcraftCharacterController characterController;
        [SerializeField] private UnitAttributeDefinition unitAttributeDefinition;
        [SerializeField] private List<UnitBehaviour> unitBehaviours;
        ...
        private readonly BehaviourController behaviourController = new();
        ...
        internal void HandleVehicleApplicationExit()
        {
            ...
            var topCheck = Mathf.Abs(UnitCollider.bounds.max.y) / 2;
            var safeExtentsY = Mathf.Abs(UnitCollider.bounds.extents.y);
            var safeExtentsX = Mathf.Abs(UnitCollider.bounds.extents.x);
            var distance = UnitCollider.height;
            ...
            {
                targetPosition -= 2f * topCheck * Vector3.up;
            }
        ...
        internal void UpdateControlState(UnitControlState state, bool applied)
        {
            ...
            var hadControl = Motion.HasMovementControl;
            ...
            var hasControl = !HasAnyState(UnitControlState.LostControl);
