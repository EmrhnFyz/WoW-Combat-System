...
namespace Client
{
    public sealed class UnitModel : MonoBehaviour
    {
        ...
        private readonly Dictionary<Material, Material> sharedMaterialsByInstancedTransparentMaterials = new();
        ...
        public void TriggerInstantCast(SpellInfo spellInfo)
        {
            ...
            var animationHash = Animator.HasState(0, animationInfo.StateHash)
        ...
        {
            var isUnitTransperent = unitRenderer.Unit.VisualEffects.HasAnyFlag(UnitVisualEffectFlags.AnyTransparency);
            if (isUnitTransperent && (!unitHasTransparency || forced))
            {
                var targetAlpha = rendering.UnitRendererSettings.StealthTransparencyAlpha;
                var currentAlpha = instantly ? targetAlpha : CurrentAlpha;
        ...
        {
            foreach (KeyValuePair<Material, Material> materialEntry in sharedMaterialsByInstancedTransparentMaterials)
            {
        ...
        private void DestroyTransparentMaterials()
        {
            if (sharedMaterialsByInstancedTransparentMaterials.Count > 0)
            {
                ...
                foreach (KeyValuePair<Material, Material> materialInstanceEntry in sharedMaterialsByInstancedTransparentMaterials)
                {
        ...
        private void UpdateAnimations(UnitRenderer unitRenderer, float deltaTime)
        {
            ...
            var isFlying = unitRenderer.Unit.HasMovementFlag(MovementFlags.Flying);
            var isCharging = unitRenderer.Unit.HasMovementFlag(MovementFlags.Charging);

            if (!isFlying || isCharging)
            {
                Animator.SetBool("Grounded", !isFlying);

                var currentStrafe = Animator.GetFloat("Strafe");
                var strafeTarget = unitRenderer.Unit.HasMovementFlag(MovementFlags.StrafeLeft) ? 0 :
                    unitRenderer.Unit.HasMovementFlag(MovementFlags.StrafeRight) ? 1 : 0.5f;

                var strafeDelta = 2 * Mathf.Sign(strafeTarget - currentStrafe) * deltaTime * strafeSpeed;
                var resultStrafe = Mathf.Clamp(currentStrafe + strafeDelta, 0.0f, 1.0f);
