    ...
    public partial class TargetingReference
    {
        private class PlayerTargetSelector : IUnitVisitor, IDisposable
        {
            ...
            private void VisitUnit(Unit unit)
            {
                ...
                {
                    var refererDirection = Vector3.ProjectOnPlane(referer.transform.forward, Vector3.up);
                    var targetDirection = Vector3.ProjectOnPlane(unit.Position - referer.Position, Vector3.up);
            ...
            private void SelectBestTarget(Unit nextTarget)
            {
                ...
                {
                    case TargetingDistance.Any:
                        var unitPreviousIndex = previousTargets.IndexOf(nextTarget);
                        ...
                        break;
                    case TargetingDistance.Near:
                        var nextTargetNearDistance = referer.ExactDistanceTo(nextTarget);
                        ...
                        break;
                    case TargetingDistance.Far:
                        var nextTargetFarDistance = referer.ExactDistanceTo(nextTarget);
