...
namespace Common
{
    ...
    public class GameObjectPool : ScriptableReference
    {
        ...
        private readonly Dictionary<int, Stack<GameObject>> pooledGameObjectsByProtoId = new();
        private readonly Dictionary<GameObject, int> takenObjectProtoIds = new();
        ...
        {
            foreach (KeyValuePair<int, Stack<GameObject>> pooledObjects in pooledGameObjectsByProtoId)
            {
                foreach (GameObject pooledObject in pooledObjects.Value)
                {
        ...
        {
            if (!takenObjectProtoIds.TryGetValue(returnedObject, out var protoId))
            {
                return false;
            }
        ...
        {
            var protoId = prototype.GetInstanceID();
        ...
        private GameObject TakeIfAvailable(int protoId, Vector3 position, Quaternion rotation, Transform parent)
        {
            if (pooledGameObjectsByProtoId.TryGetValue(protoId, out Stack<GameObject> pooledObjects))
            {
                while (pooledObjects.Count > 0)
                {
                    ...
                    objectToTake.transform.SetPositionAndRotation(position, rotation);
                    objectToTake.transform.SetParent(parent != null ? parent : container);
        ...
        public static void PreInstantiate(GameObject prototype, int preinstantiatedCount)
        {
            ...
            for (var i = existingCount; i < preinstantiatedCount; i++)
            {
        ...
        {
            var returnedSuccessfully = Instance?.ProcessReturn(takenObject, destroyed) ?? false;
