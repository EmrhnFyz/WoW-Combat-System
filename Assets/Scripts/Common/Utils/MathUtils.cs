...
namespace Common
{
    public static class MathUtils
    {
        public static bool CompareWith(this float self, float other, ComparisonOperator comparison)
        {
            ...
            {
                ComparisonOperator.Equal => Mathf.Approximately(self, other),
                ComparisonOperator.NotEqual => !Mathf.Approximately(self, other),
                ComparisonOperator.Less => self < other,
                ComparisonOperator.LessOrEqual => self <= other,
                ComparisonOperator.Greater => self > other,
                ComparisonOperator.GreaterOrEqual => self >= other,
                _ => throw new ArgumentOutOfRangeException(nameof(comparison), comparison, "Unknown comparison type!"),
            };
        }
